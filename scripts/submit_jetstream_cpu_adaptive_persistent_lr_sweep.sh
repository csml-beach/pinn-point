...
usage() {
  cat <<'EOF'
Usage:
  submit_jetstream_cpu_adaptive_persistent_lr_sweep.sh [--parallel N] [--threads N] [--commit SHA] [--config PATH] [--sync-root PATH]

Description:
  Submit a 3-seed Jetstream CPU learning-rate sweep for adaptive_persistent on
...
if [[ -z "$sync_root" ]]; then
  sync_root="$repo_root/outputs/jetstream-medium-adaptive-persistent-lr-sweep"
fi
...
runner_dir="/tmp/pinn_point_jetstream_adaptive_persistent_lr_sweep_runners"
...
submit_job() {
  ...
  local lr_tag
  lr_tag="$(echo "$lr" | tr '.' 'p')"
  local session="cpu-adaptpersist-lr${lr_tag}-seed${seed}"
...
EOF
  ...
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] submit seed=${seed} lr=${lr} session=${session}"
  remote_exec "
    set -euo pipefail
    rm -f '$remote_log'
  ...
  if [[ -z "$run_id" ]]; then
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] failed seed=${seed} lr=${lr}: could not resolve run_id" >&2
    return 1
  fi
...
JSON
  ...
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] done seed=${seed} lr=${lr} run_id=${run_id} exit_code=${exit_code}"
}
...
wait
echo "[submit] all sweep jobs completed"
